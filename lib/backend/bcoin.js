...
bitcoin.listReceivedByAddress = function(addr, callback) {
  this.getTransactions(function(err, txs) {
    if (err) return callback(err);
    var mytxs = txs.filter(function(tx) {
      return (tx.category === 'receive' && tx.address === addr)
            || (tx.category === 'move' && tx.details[0].address === addr);
    });
    callback(null, mytxs);
  });
}
